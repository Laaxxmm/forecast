...
import {
  RowMeta,
  TOTAL_ROW_LABELS as SHARED_TOTAL_ROW_LABELS,
  SECTION_HEADER_LABELS as SHARED_SECTION_HEADER_LABELS,
  PERCENT_ROW_LABELS as SHARED_PERCENT_ROW_LABELS,
  sumCat,
  formatNum,
  buildPnLRows,
...
// Row-classification label sets now live in
// client/src/utils/financialStatements.ts so they can be reused by Compare /
// What-if / Report views. Aliased here so existing references resolve.
const TOTAL_ROW_LABELS = SHARED_TOTAL_ROW_LABELS;
const SECTION_HEADER_LABELS = SHARED_SECTION_HEADER_LABELS;
const PERCENT_ROW_LABELS = SHARED_PERCENT_ROW_LABELS;

// sumCat lives in financialStatements.ts and is imported above.

// formatNum lives in financialStatements.ts and is imported above. The local
// stub below keeps a sentinel reference so removing the comment block never
// silently strips the import (TS will catch it).
// formatNum & sumCat live in financialStatements.ts (imported above).
...
// RowMeta, buildPnLRows, buildBalanceSheetRows, buildCashFlowRows now live in
// client/src/utils/financialStatements.ts (imported above) so they can be
// reused by the Scenario Analysis Compare / What-if / Report views.
