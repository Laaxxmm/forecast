...
export default function CategoryTab({ category, label, scenario, months, viewMode, items, allItems, allValues, settings, onReload, readOnly }: Props) {
  ...
  const handleInlineAdd = async () => {
    ...
    const defaultTypes: Record<string, string> = { revenue: 'revenue_only', direct_costs: 'general_cost' };
    const res = await api.post('/forecast-module/items', {
      scenario_id: scenario.id,
      category,
      name: inlineAddName.trim(),
      item_type: defaultTypes[category] || config.itemTypes[0]?.value || 'other',
      entry_mode: 'varying',
      start_month: months[0],
    });
