    ...
    <div>
        ...
        <div className="flex items-center gap-2">
          {items.length > 0 && (
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors border border-slate-200"
      ...
      {config.showChart && items.length > 0 && (
        <div className="card mb-4">
          <h3 className="text-sm font-semibold text-slate-600 mb-3">{label} Totals</h3>
          <div className="h-48">
          ...
          <tbody>
            ...
            {config.addLabel && (
              <tr className="border-b border-slate-100">
                <td className="py-2.5 px-4 sticky left-0 bg-white z-10" colSpan={months.length + 2}>
                  <button
                    onClick={handleAddClick}
...
function CashFlowAssumptionsTab({ items: _items, months: _months, allValues: _allValues }: { items: ForecastItem[]; months: string[]; allValues: Record<number, Record<string, number>> }) {
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-800 mb-4">Cash Flow Assumptions</h2>
      <div className="card">
        <h3 className="font-semibold text-slate-700 mb-3">Accounts Receivable</h3>
        <p className="text-sm text-slate-500 mb-4">Configure how quickly you collect payments from customers.</p>
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="font-medium text-slate-600">Revenue Stream</div>
          <div className="font-medium text-slate-600">Sales on Credit (%)</div>
          <div className="font-medium text-slate-600">Days to Get Paid</div>
      ...
      <div className="card mt-4">
        <h3 className="font-semibold text-slate-700 mb-3">Accounts Payable</h3>
        <p className="text-sm text-slate-500 mb-4">Configure how quickly you pay your vendors and suppliers.</p>
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="font-medium text-slate-600">Expense</div>
          <div className="font-medium text-slate-600">Purchases on Credit (%)</div>
          <div className="font-medium text-slate-600">Days to Pay</div>
          <div className="text-slate-500">Default</div>
          <input type="number" defaultValue={0} className="input py-1.5 text-sm" />
          <select className="input py-1.5 text-sm"><option>30 days</option><option>15 days</option><option>45 days</option><option>60 days</option><option>90 days</option></select>
        </div>
...
function InitialBalancesTab() {
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-800 mb-2">Initial Balances</h2>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card text-center">
          ...
          <div className="text-2xl font-bold text-slate-800 mt-1">Rs0</div>
        </div>
        <div className="card text-center">
      ...
      <div className="flex gap-2 mb-4">
        {['Initial Assets', 'Initial Liabilities', 'Total Equity'].map(tab => (
          <button key={tab} className="px-4 py-2 text-sm font-medium rounded-lg border border-slate-200 hover:bg-slate-50">{tab}</button>
        ))}
      </div>
      <div className="card">
