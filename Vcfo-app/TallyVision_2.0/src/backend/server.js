...
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'http://localhost:3456').split(',').map(s => s.trim());
app.use(cors({
    origin: (origin, cb) => {
        if (!origin || ALLOWED_ORIGINS.includes(origin) || ALLOWED_ORIGINS.includes('*')) cb(null, true);
        else cb(null, false);
    },
    credentials: true,
...
app.get('/client/login', (req, res) => {
    if (isClientSession(req)) return res.redirect((req.baseUrl || '') + '/client');
...
app.all('/client/logout', (req, res) => {
    ...
    res.redirect((req.baseUrl || '') + '/client/login');
});

app.get('/client', (req, res) => {
    if (!isClientSession(req)) return res.redirect((req.baseUrl || '') + '/client/login');
    ...
    // Safely encode data to prevent XSS via </script> in displayName
    const safeJson = (v) => JSON.stringify(v).replace(/</g, '\\u003c').replace(/>/g, '\\u003e');
    const inject = `<script>window.TV_ROLE='client';window.TV_CLIENT_COMPANIES=${safeJson(clientData.companyIds)};window.TV_CLIENT_NAME=${safeJson(clientData.displayName || clientData.username)};window.TV_CLIENT_FEATURES=${safeJson(clientData.features || {})};</script>`;
...
// ===== COMBINED INIT (companies + groups + settings + geo filters in 1 call) =====
...
app.get('/api/filters/states', (req, res) => {
    ...
    let scopeIds = groupId ? resolveGroupMemberIds(Number(groupId)) : null;
    // Client users: intersect with their allowed companies
    if (isClientSession(req)) {
...
app.get('/api/dashboard/kpi', (req, res) => {
    ...
    function computeFullKPI(f, t) {
        const kpi = computeKPIData(ids, f, t);
        if (groupId && ids.length > 1) {
            const adjusted = applyDashboardAllocations(ids, groupId, f, t);
        ...
        if (groupId) {
            ...
            if (woRules) {
                const units = ids.map(cid => {
                    const k = computeKPIData([cid], f, t);
                    return { companyId: cid, ...k };
                });
                if (ids.length > 1) {
                    const allocRules = getDashboardAllocRules(groupId);
                    if (allocRules) applyAllocRulesToUnits(units, allocRules, f, t);
                }
                applyWriteoffRulesToUnits(units, woRules, f, t);
                const sum = (key) => units.reduce((s, u) => s + (u[key] || 0), 0);
                Object.assign(kpi, { indirectExpenses: sum('indirectExpenses'), indirectIncome: sum('indirectIncome'), netProfit: sum('netProfit'), _writeoffsApplied: true });
            }
        }
        return kpi;
    }

    const kpi = computeFullKPI(from, to);

    // Optional: compute previous period KPI in same request to avoid 2nd round trip
    const prevFrom = req.query.prevFromDate;
    const prevTo = req.query.prevToDate;
    if (prevFrom && prevTo) {
        const prevKpi = computeFullKPI(prevFrom, prevTo);
        return res.json({ current: kpi, previous: prevKpi });
    }
...
app.get('/api/dashboard/monthly-trend', (req, res) => {
    ...
    // Apply dashboard-affecting allocation rules per month (optimized: use per-company trend, not full KPI)
    const groupId = Number(req.query.groupId);
    if (groupId && ids.length > 1) {
        const rules = getDashboardAllocRules(groupId);
        if (rules) {
            // Pre-compute per-company monthly trends in ONE batch query
            const perCompanyTrends = {};
            for (const cid of ids) {
                perCompanyTrends[cid] = {};
                const ct = computeMonthlyTrendData([cid], from, to);
                for (const m of ct) perCompanyTrends[cid][m.month] = m;
            }
            ...
            for (const monthData of trend) {
                ...
                // Build lightweight per-company units from pre-computed trends
                const units = ids.map(cid => {
                    const mt = perCompanyTrends[cid][mFrom] || {};
                    return {
                        companyId: cid,
                        revenue: mt.revenue || 0,
                        directIncome: mt.directIncome || 0,
                        indirectExpenses: mt.indirectExpenses || 0,
                        indirectIncome: mt.indirectIncome || 0,
                        grossProfit: mt.grossProfit || 0,
                        netProfit: mt.netProfit || 0,
                    };
                });
                ...
                monthData.indirectExpenses += (postIndExp - preIndExp);
                monthData.indirectIncome += (postIndInc - preIndInc);
...
// ===== COMBINED DASHBOARD (single request, shared queries) =====
app.get('/api/dashboard/combined', (req, res) => {
    const ids = resolveIds(req);
    if (!ids) return res.status(400).json({ error: 'companyId, companyIds, or geographic filters required' });
    const from = req.query.fromDate || '2024-04-01';
    const to   = req.query.toDate   || '2025-03-31';
    ...
    // Shared queries — computed ONCE
    const lgMap = buildLedgerGroupMap(ids);
    const plSets = buildPLGroupSets(ids);
    const tbLedgers = getLedgerFlowsTB(ids, from, to);

    // Top revenue (shared lgMap + tbLedgers + plSets)
    const topRev = getTopLedgers(tbLedgers, lgMap, plSets.directCredit, 10, false);

    // Top direct expenses
    const topDirect = getTotalsByGroup(tbLedgers, lgMap, plSets.directDebit, true).slice(0, 10);

    // Top indirect expenses
    const topIndirect = getTotalsByGroup(tbLedgers, lgMap, plSets.indirectDebit, true).slice(0, 10);

    // Allocation enrichment (compute per-company KPIs ONCE, reuse for all)
    let allocUnits = null;
    if (groupId && ids.length > 1) {
        const rules = getDashboardAllocRules(groupId);
        if (rules) {
            allocUnits = ids.map(cid => {
                const kpi = computeKPIData([cid], from, to);
                return { companyId: cid, ...kpi };
            });
            // Enrich top-revenue with allocation income
            for (const rule of rules) {
                const config = JSON.parse(rule.config || '{}');
                if (rule.rule_type === 'percent_income') {
                    const pct = (Number(config.percentage) || 0) / 100;
                    ...
                    for (const sid of sourceIds) {
                        const src = allocUnits.find(u => u.companyId === sid);
                        if (src) {
                            totalIncome += (src.directIncome || src.revenue || 0) * pct;
                            totalCharge += (src.directIncome || src.revenue || 0) * pct;
                        }
                    }
                    if (totalIncome > 0) topRev.push({ ledger_name: config.income_label || 'Franchise Income', group_name: 'Indirect Income (Allocation)', total: totalIncome, _allocation: true });
                    if (totalCharge > 0) topIndirect.push({ category: config.expense_label || 'HO Charges', total: totalCharge, _allocation: true });
                } else if (rule.rule_type === 'fixed' || rule.rule_type === 'ratio') {
                    const config2 = JSON.parse(rule.config || '{}');
                    const overlap = getEffectiveOverlap(config2, from, to);
                    if (overlap && overlap.overlaps) {
                        topIndirect.push({ category: `Alloc: ${rule.rule_name}`, total: 0, _allocation: true, _note: 'Internal transfer (net zero)' });
                    }
                }
            }
            topRev.sort((a, b) => b.total - a.total);
            topIndirect.sort((a, b) => b.total - a.total);
        }
    }

    res.json({
        topRevenue: topRev.slice(0, 10),
        topDirectExpenses: topDirect,
        topIndirectExpenses: topIndirect.slice(0, 10)
    });
});
...
app.post('/api/allocation-rules', adminOnly, (req, res) => {
...
app.post('/api/writeoff-rules', adminOnly, (req, res) => {
...
// ===== MONTHLY FINANCIAL REPORT HELPERS =====

function getFYMonths(fy) {
    // fy = "2026-27" → ["2026-04","2026-05",...,"2027-03"]
    const startYear = parseInt(fy.split('-')[0]);
    const months = [];
    for (let i = 0; i < 12; i++) {
        const m = ((3 + i) % 12) + 1; // Apr=4, May=5, ..., Mar=3
...
// ── Endpoint: Monthly Profit & Loss ──
app.get('/api/reports/profit-loss-monthly', (req, res) => {
    const ids = resolveIds(req);
    if (!ids) return res.status(400).json({ error: 'No companies selected' });
    const fy = req.query.fy;
    if (!fy) return res.status(400).json({ error: 'fy parameter required (e.g. 2026-27)' });

    try {
        const months = getFYMonths(fy);

        const sectionDefs = [
            { key: 'revenue',          label: 'Revenue (Sales)',   field: 'revenue',          isExpense: false },
            { key: 'directIncome',     label: 'Direct Income',     field: 'directIncome',     isExpense: false },
            { key: 'purchase',         label: 'COGS (Purchase)',   field: 'purchase',          isExpense: true },
            { key: 'directExpenses',   label: 'Direct Expenses',   field: 'directExpenses',   isExpense: true },
            { key: 'indirectExpenses', label: 'Indirect Expenses', field: 'indirectExpenses', isExpense: true },
            { key: 'indirectIncome',   label: 'Indirect Income',   field: 'indirectIncome',   isExpense: false },
        ];

        // Compute KPI data for each month
        const monthlyKPI = {};
        for (const m of months) {
            const { from, to } = getMonthRange(m);
            monthlyKPI[m] = computeKPIData(ids, from, to);
        }
        ...
        const sections = sectionDefs.map(def => {
            ...
            let grandTotal = 0;
            for (const m of months) {
                const val = monthlyKPI[m][def.field] || 0;
                totals[m] = val;
                grandTotal += val;
            }
            return { key: def.key, label: def.label, isExpense: def.isExpense, totals, grandTotal };
        });

        // Computed rows
        const grossProfit = {}, grossMargin = {}, operatingExpenses = {}, otherIncome = {}, netProfit = {};
        let gpTotal = 0, npTotal = 0;

        for (const m of months) {
            const kpi = monthlyKPI[m];
            const gp = kpi.grossProfit || 0;
            const np = kpi.netProfit || 0;
            const revPlusDirectInc = (kpi.revenue || 0) + (kpi.directIncome || 0);

            grossProfit[m] = gp;
            grossMargin[m] = revPlusDirectInc !== 0 ? Math.round((gp / revPlusDirectInc) * 10000) / 100 : 0;
            operatingExpenses[m] = kpi.indirectExpenses || 0;
            otherIncome[m] = kpi.indirectIncome || 0;
            netProfit[m] = np;
        ...
        res.json({
            ...
            months,
            sections,
            computed: { grossProfit, grossMargin, operatingExpenses, otherIncome, netProfit },
            grandTotals: { grossProfit: gpTotal, netProfit: npTotal },
        });
    } catch (err) {
        console.error('PL-monthly error:', err);
        res.status(500).json({ error: err.message });
    }
});

// ── Endpoint: Monthly Balance Sheet ──
app.get('/api/reports/balance-sheet-monthly', (req, res) => {
    const ids = resolveIds(req);
    if (!ids) return res.status(400).json({ error: 'No companies selected' });
    const fy = req.query.fy;
    if (!fy) return res.status(400).json({ error: 'fy parameter required (e.g. 2026-27)' });
    ...
    try {
        ...
        // Pre-resolve group trees (they don't change per month)
        const groupTrees = {};
        for (const sec of BS_SECTIONS) {
            groupTrees[sec.key] = getGroupTree(ids, sec.parent);
        }

        const sections = [];
        const totalAssets = {};
        const totalLiabilities = {};
        for (const m of months) { totalAssets[m] = 0; totalLiabilities[m] = 0; }

        for (const sec of BS_SECTIONS) {
            const groups = groupTrees[sec.key];
            if (!groups.length) continue;
            ...
            for (const m of months) {
                const { to } = getMonthRange(m);
                const closing = computeBSClosing(ids, to, groups, null, lgMap);
                const amount = sec.side === 'asset' ? -closing : closing;
                values[m] = amount;
                if (Math.abs(amount) >= 0.01) hasAnyValue = true;
                if (sec.side === 'asset') totalAssets[m] += amount;
                else totalLiabilities[m] += amount;
            }

            if (hasAnyValue) {
                sections.push({ key: sec.key, label: sec.label, side: sec.side, values });
            }
        }

        // Add P&L A/c (retained earnings) to liabilities for each month
        const fyStart = `${parseInt(fy.split('-')[0])}-04-01`;
        const pnlValues = {};
        let hasPnl = false;
        for (const m of months) {
            const { to } = getMonthRange(m);
            const kpi = computeKPIData(ids, fyStart, to);
            const np = kpi.netProfit || 0;
            pnlValues[m] = np;
            totalLiabilities[m] += np;
            if (Math.abs(np) >= 0.01) hasPnl = true;
        }
        if (hasPnl) {
            sections.push({ key: 'pnl', label: 'Profit & Loss A/c', side: 'liability', values: pnlValues });
        }

        res.json({ fy, months, sections, totalAssets, totalLiabilities });
    } catch (err) {
        console.error('BS-monthly error:', err);
        res.status(500).json({ error: err.message });
    }
});

// ── Endpoint: Monthly Cash Flow ──
app.get('/api/reports/cash-flow-monthly', (req, res) => {
    const ids = resolveIds(req);
    if (!ids) return res.status(400).json({ error: 'No companies selected' });
    const fy = req.query.fy;
    if (!fy) return res.status(400).json({ error: 'fy parameter required (e.g. 2026-27)' });
    ...
    try {
        ...
        const finLabels = [
            'Inc/(Dec) in Secured Loans',
            'Inc/(Dec) in Unsecured Loans',
            'Inc/(Dec) in Capital',
        ];

        // Initialize result structures
        const operating = opLabels.map(label => ({ label, values: {}, total: 0 }));
        const investing = invLabels.map(label => ({ label, values: {}, total: 0 }));
        const financing = finLabels.map(label => ({ label, values: {}, total: 0 }));
        const operatingTotals = {}, investingTotals = {}, financingTotals = {};
        const netChange = {}, openingCash = {}, closingCash = {};

        for (const m of months) {
            const { from, to } = getMonthRange(m);
            const kpi = computeKPIData(ids, from, to);

            // Opening date = day before month start
            const fromDate = new Date(from + 'T00:00:00');
            fromDate.setDate(fromDate.getDate() - 1);
            const openingDate = fromDate.toISOString().split('T')[0];

            // Operating
            const opNetProfit = kpi.netProfit || 0;
            ...
            const opAmounts = [
                ...
                closingCreditors - openingCreditors,
                openingStockBal - closingStockBal,
            ];

            let opTotal = 0;
            opAmounts.forEach((amt, i) => {
                operating[i].values[m] = amt;
                operating[i].total += amt;
                opTotal += amt;
            });
            operatingTotals[m] = opTotal;
            ...
            const invAmounts = [
                openingFA - closingFA,
                openingInv - closingInv,
            ];

            let invTotal = 0;
            invAmounts.forEach((amt, i) => {
                investing[i].values[m] = amt;
                investing[i].total += amt;
                invTotal += amt;
            });
            investingTotals[m] = invTotal;
            ...
            const finAmounts = [
                closingSL - openingSL,
                closingUL - openingUL,
                closingCap - openingCap,
            ];

            let finTotal = 0;
            finAmounts.forEach((amt, i) => {
                financing[i].values[m] = amt;
                financing[i].total += amt;
                finTotal += amt;
            });
            financingTotals[m] = finTotal;

            // Net change & cash balances
            netChange[m] = opTotal + invTotal + finTotal;

            const openingCashRaw = computeBSClosing(ids, openingDate, allCashGroups, null, lgMap);
            openingCash[m] = -openingCashRaw;
            closingCash[m] = kpi.cashBankBalance;
        }
        ...
        console.error('CF-monthly error:', err);
        res.status(500).json({ error: err.message });
    }
});
...
// Upload budget Excel
app.post('/api/budgets/upload', adminOnly, multer({ storage: multer.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 }, fileFilter: (req, file, cb) => { const ext = path.extname(file.originalname).toLowerCase(); cb(null, ext === '.xlsx' || ext === '.xls'); } }).single('file'), async (req, res) => {
...
app.post('/api/upload/excel', adminOnly, upload.single('file'), async (req, res) => {
    try {
        ...
        // Validate category (also check for path traversal chars)
        if (/[\/\\\.]{2}/.test(category)) {
...
app.post('/api/cloud/publish', adminOnly, async (req, res) => {
...
app.post('/api/vcfo/seed', adminOnly, (req, res) => {
    const groupId = Number(req.query.groupId || req.body.groupId);
    if (!groupId) return res.status(400).json({ error: 'groupId required' });
...
// ── VCFO: Create item ──
app.post('/api/vcfo/items', adminOnly, (req, res) => {
...
// ── VCFO: Update item ──
app.put('/api/vcfo/items/:id', adminOnly, (req, res) => {
...
// ── VCFO: Delete item ──
app.delete('/api/vcfo/items/:id', adminOnly, (req, res) => {
...
app.get('/api/vcfo/status', (req, res) => {
    ...
    if (period && items.length && memberIds.length) {
        ...
        statusRows = db.prepare(`SELECT * FROM vcfo_tracker_status WHERE item_id IN (${idPh(itemIds)}) AND company_id IN (${idPh(memberIds)}) AND period_key=?`).all(...itemIds, ...memberIds, period);
    }
...
app.put('/api/vcfo/status', adminOnly, (req, res) => {
...
app.post('/api/vcfo/status/bulk', adminOnly, (req, res) => {
...
app.get('/api/vcfo/summary', (req, res) => {
    ...
    const itemIds = items.map(i => i.id);
    const counts = db.prepare(`SELECT status, COUNT(*) as cnt FROM vcfo_tracker_status WHERE item_id IN (${idPh(itemIds)}) AND company_id IN (${idPh(memberIds)}) AND period_key=? GROUP BY status`).all(...itemIds, ...memberIds, period);
    const map = {};
...
// When required as a sub-app (mounted by Magna_Tracker at /vcfo), the parent
// server owns the HTTP listener and lifecycle, so skip app.listen() here.
