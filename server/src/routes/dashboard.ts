...
router.get('/overview', async (req, res) => {
  ...
  const clinicSummary = db.get(
    ...
    FROM clinic_actuals WHERE bill_month >= ? AND bill_month <= ?`,
    startMonth, endMonth
  ...
  const pharmaSummary = db.get(
    ...
    FROM pharmacy_sales_actuals WHERE bill_month >= ? AND bill_month <= ?`,
    startMonth, endMonth
  ...
  const clinicBudget = db.get(
    "SELECT COALESCE(SUM(amount), 0) as total FROM budgets WHERE fy_id = ? AND business_unit = 'CLINIC' AND metric = 'revenue'",
    fy.id
  );
  const pharmaBudget = db.get(
    "SELECT COALESCE(SUM(amount), 0) as total FROM budgets WHERE fy_id = ? AND business_unit = 'PHARMACY' AND metric = 'sales_amount'",
    fy.id
  );
...
router.get('/variance', async (req, res) => {
  ...
  // Find the default scenario for this FY
  const scenario = db.get(
    'SELECT id FROM scenarios WHERE fy_id = ? AND is_default = 1 LIMIT 1',
    fy_id
  );
  ...
  // since forecast items aren't broken down by department
